    ...
    if os.path.isdir(local_file_name):
        ...
        if not message.photo:
            new_m_esg = await message.reply_text(
                f"🔎FOUND {len(directory_contents)} FILES <a href='tg://user?id={from_user}'>🗂️</a>",
                quote=True
...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    del_it = await message.edit_text(
        f"<a href='tg://user?id={g_id}'>📤</a>👻NOW UPLOADING TO ☁️ GOOGLE DRIVE!!🔁 BY @thesilentninja"
    )
    ...
    if os.path.isfile(file_upload):
        ...
        button.append(
            [pyrogram.InlineKeyboardButton(
                text="☁️ GDRIVE-URL ☁️", url=f"{gauti}")]
        )
        if INDEX_LINK:
        ...
        button.append(
            [pyrogram.InlineKeyboardButton(
                text="☁️ GDRIVE-URL ☁️", url=f"{gautii}")]
        )
        if INDEX_LINK:
            indexurl = f"{INDEX_LINK}/{os.path.basename(file_upload)}/"
            tam_link = requests.utils.requote_uri(indexurl)
